...
public class TreeTable extends Composite {
   ...
   */
  public TreeTable(
      final TreeTableModel model, Tree.Resources images, CellFormatter formatter, boolean showRoot) {
    this.model = model;
    this.images = (images == null) ? GWT.<Tree.Resources>create(Tree.Resources.class) : images;
  ...
  public class TreeTableItem {
    ...
    TreeTableItem(Object node, RowAccountingTable.Row row, int level) {
      ...
      if (level < 0) { 
        ...
        widget.setImage(AbstractImagePrototype.create(images.treeLeaf()));
    ...
    TreeTableItem addChild(int index, Object node) {
      ...
      if (children.size() == 0) {
        ...
        before = row.next();
        if (widget != null) {
          widget.setImage(AbstractImagePrototype.create(open ? images.treeOpen() : images.treeClosed()));
        }
        index = 0;
      } else if (index == 0) {
